  ...
  const services = [
    {
      ...
      title: "UI/UX Design",
      description:
        "Crafting intuitive and user-friendly interfaces that focus on seamless navigation and engaging digital experiences."
    },
    {
      icon: <Monitor className="h-8 w-8 text-yellow-500" />,
      title: "Application Design",
      description:
        "Designing modern, scalable applications with clean layouts and efficient workflows tailored to business requirements."
    },
    {
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {services.map((service, index) => (
            <div key={index} className="bg-white rounded-2xl p-8 text-center  duration-300  hover:shadow-lg transition-shadow">
              <div className="w-16 h-16 mx-auto mb-6 bg-gray-50 rounded-2xl flex items-center justify-center">
                {service.icon}
              </div>
              <h3 className="text-xl font-semibold mb-4">{service.title}</h3>
              <p className="text-gray-600 mb-6">{service.description}</p>
              <Button variant="link" className="text-yellow-500 cursor-pointer p-0">
                Learn More →
              </Button>
            </div>
          ))}
